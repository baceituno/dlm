...
class FinDiffSim(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(20):
            ...
            y_p, p_p = forwardPlanarSim(inp, in1, in2, False, in4)
            y_m, p_m = forwardPlanarSim(inm, in1, in2, False, in4)
...
class FinDiffSagSim(torch.autograd.Function):
    ...
    def forward(ctx, in0, in1, in2, in3, in4):
        ctx.save_for_backward(in0, in1, in2, torch.tensor(in4))
        y, p =  forwardSaggitalSim(in0, in1, in2, 0.0, 0.0, in3, in4)
        return torch.cat((torch.tensor(y).view(1,-1), torch.tensor(p).view(1,-1)), axis = 1)
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(20):
            ...
            y_p, p_p = forwardSaggitalSim(inp, in1, in2, 0.0, 0.0, False, in4)
            y_m, p_m = forwardSaggitalSim(inm, in1, in2, 0.0, 0.0, False, in4)
...
def forwardPlanarSim(p, polygon, xtraj0, render = False, gamma = 0.01):
    bodies = []
    joints = []
    facets = []
    ...
    r0 = Hull([xr, yr], verts, restitution=0, fric_coeff=10, mass = 0.99, name="obj")
    r0.name = "obj"
    ...
    for i in range(n_pol):
        ...
        r1 = Hull(p0, verts, mass = 0.01/n_pol, restitution=restitution, fric_coeff=10, name="obj_"+str(i))
    ...
    for i in range(2):
        ...
        c = Circle(pos0, 10, vel=(0, 0, 0), restitution=restitution, fric_coeff=100, name = "f"+str(i))
        c.name = "f"+str(i)
        c.col = (255,0,0)
    ...
    world = World(bodies, joints, dt=0.02, tol = 1e-6, eps = float('inf'), post_stab = False, strict_no_penetration = False)
    world.gamma = gamma
    screen = None
    if render:
        pygame.init()
    ...
    run_world(world, run_time = 0.61, screen=screen, recorder=None, print_time=False, traj=traj_f)

    for t in range(5):
        if t > 0:
            y0 = torch.cat((world.states[t][1].view(1,1) - 500, 500 - world.states[t][2].view(1,1), -scale*torch.sin(world.states[t][0].view(1,1))*0.01), axis = 0)/scale
            ...
            y = torch.cat((world.states[t][1].view(1,1) - 500, 500 - world.states[t][2].view(1,1), -scale*torch.sin(world.states[t][0].view(1,1))*0.01), axis = 0)/scale
...
def forwardSaggitalSim(p, polygon, xtraj0, angle = 0.0, height = 0.0, render = False, gamma = 0.01):
    bodies = []
    joints = []
    facets = []
    restitution = 0 # no impacts in quasi-dynamics
    ...
    for i in range(n_pol):
        ...
        if cvx:
            ...
            r1 = Hull(p0, verts, mass = 0.01/n_pol, restitution=restitution, fric_coeff=100, name="obj_"+str(i))
            r1.name = "obj_"+str(i)
            r1.col = (239, 188, 64)
            for j in range(i+1):
                r1.add_no_contact(bodies[j])
    ...
    if cvx:
        ...
        r0 = NonConvex([xr, yr], np.array(ordered_verts), restitution=restitution, fric_coeff=10, mass = 1, name="obj")
        r0.name = "obj"
        r0.add_force(Gravity(g=9.8))
        r0.col = (239, 188, 64)
        bodies.append(r0)
    ...
    for i in range(2):
        pos0 = [500 + scale*p[i,0], 500 - scale*p[i+2,0]]
        c = Circle(pos0, 10, vel=(0, 0, 0), mass = 10, restitution=restitution, fric_coeff=100, name = "f"+str(i))
        c.name = "f"+str(i)
        c.col = (255,0,0)
        traj = torch.cat((scale*dp[i,:],-scale*dp[i+2,:]), axis=0).view(2,5)
        traj_f.append(Trajectory(vel = traj, name = "f"+str(i)))
    ...
    r = Rect([0, 500, 500 + width - scale*height], [1200, 2*width], mass = 0.01, restitution=restitution, fric_coeff=100, name = "env")
    r.name = "env"
    r.col = (0,0,0)
    ...
    world = World(bodies, joints, dt=0.005, tol = 1e-6, eps = float('inf'), post_stab = False, strict_no_penetration = False, facets = facets)
    world.gamma = gamma
