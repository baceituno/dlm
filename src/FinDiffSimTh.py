...
class FinDiffSimTheta(torch.autograd.Function):

    @staticmethod
    def forward(ctx, in0, in1, in2, in3, in4):
        ctx.save_for_backward(in0, in1, in2, torch.tensor(in4))
        y, p = forwardPlanarSimTheta(in0, in1, in2, in3, in4)
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(20):
            ...
            y_p, p_p = forwardPlanarSimTheta(inp, in1, in2, False, in4, float('inf'))
            y_m, p_m = forwardPlanarSimTheta(inm, in1, in2, False, in4, float('inf'))
...
class FinDiffSagSimTheta(torch.autograd.Function):
    @staticmethod
    def forward(ctx, in0, in1, in2, in3, in4):
        ctx.save_for_backward(in0, in1, in2, torch.tensor(in4))
        y, p =  forwardSaggitalSimTheta(in0, in1, in2, 0.0, 0.0, in3, in4, 5)
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(20):
            ...
            y_p, p_p = forwardSaggitalSimTheta(inp, in1, in2, 0.0, 0.0, False, in4, float('inf'))
            y_m, p_m = forwardSaggitalSimTheta(inm, in1, in2, 0.0, 0.0, False, in4, float('inf'))
...
def forwardPlanarSimTheta(p, polygon, xtraj0, render = False, gamma = 0.01, eps = 0.1):
    bodies = []
    joints = []
    facets = []
...
def forwardSaggitalSimTheta(p, polygon, xtraj0, angle = 0.0, height = 0.0, render = False, gamma = 0.01, eps = 0.1):
    render = True
    bodies = []
    joints = []
    facets = []
